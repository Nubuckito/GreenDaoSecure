...
public class SQLiteDatabaseWrapper {
	...
	/**
	 * Compiles an SQL statement into a reusable pre-compiled statement object. The parameters are identical to {@link #execSQL(String)}. You may put ?s in the statement and fill in those values with {@link android.database.sqlite.SQLiteProgram#bindString} and {@link android.database.sqlite.SQLiteProgram#bindLong} each time you want to run the statement. Statements may not return result sets larger than 1x1.
	 * 
	 * @param sql
	 *            The raw SQL statement, may contain ? for unknown values to be bound later.
	 * @return A pre-compiled {@link android.database.sqlite.SQLiteStatement} object. Note that {@link android.database.sqlite.SQLiteStatement}s are not synchronized, see the documentation for more details.
	 */
	 ...
	 * @return A {@link android.database.Cursor} object, which is positioned before the first entry. Note that {@link android.database.Cursor}s are not synchronized, see the documentation for more details.
